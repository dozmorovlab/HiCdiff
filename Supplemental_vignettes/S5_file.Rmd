---
title: "Extended evaluation of differential chromatin interaction detection analysis using simulated Hi-C data"
...
The tables show that the most true differences are detected after joint `loess` normalization compared to the other normalization techniques. `loess` also has the lowest number of false positives among the normalization techniques. With `loess`, differences at smaller fold changes (1.5 and 2) are able to be detected more reliably compared to the other methods and this superiority continues to the higher fold changes though the other methods tend to make up some ground. `ChromoR` perofrmed the worst of the normalization techniques tested while `KR` and `SCN` tended to perform better. 
