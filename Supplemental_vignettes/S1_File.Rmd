...
```{r}
# load data
githubURL <- "https://github.com/dozmorovlab/HiCdiff/raw/master/Supplemental_data/S1_File_data.RData"
load(url(githubURL))
```
...
To compare the ability of methods for normalzing individual datasets to remove biases _between_ chromatin interaction matrices, we compare individually normalized matrices with the jointly normalized ones. Several parameters were assessed:
...
```{r}
diffs = hic_loess(chr11.tab, Plot=T, check.differences = T, diff.thresh = 1)
tab = diffs
counts = sum(diffs$p.value < 0.05)
print(paste0(counts, ' differences found between the datasets'))
```

The MD plot above serves as a reference to show that `Loess` can successfully normalizes the data and removes bias between the two datasets. The following MD plots display the data after the specified individual normalization method has been applied to each matrix. The dotted lines on the second MD plot represent the difference threshold where points are considered significant outside of the two dotted lines. The difference threshold used here is $\theta = 1$, see `?hic_diff` and the methods section of the paper for a full explaination on the difference threshold.
...
```{r}
loess.diffs = hic_loess(chr11.unscaled.tab, check.differences = T, Plot = T, diff.thresh = 1)
